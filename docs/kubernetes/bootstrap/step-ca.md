...
Step-ca is an online Certificate Authority (CA) for secure and automated X.509
management.

At the beginning the CA server was on a
[standalone raspberry pi server](../../archive/the_ca_server.md). After
encountering many times issues with SD cards, I decided to move it into the new
bootstrap Talos server.

- [Creating the value file](#creating-the-value-file)
- [Testing](#testing)
- [Bonus: get its own certificate](#bonus-get-its-own-certificate)
  - [OPNsense automatic certificate](#opnsense-automatic-certificate)
    - [Bind](#bind)
