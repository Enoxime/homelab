...
> [!CAUTION]
> Please don't use that in production or even for your staging env without
> reading properly the code. This setup is meant for my personal homelab and
> it is still in early development. Use it with caution.

## Description

Personal homelab setup with the magic of automation.

This project try to acomplish the creation and management of a homelab the
gitops way.
...
- [Planification](#planification)
  ...
  - [Topology](#topology)
  - [The plan](#the-plan)
    - [Setup the router](#step-one-setup-the-router)
...
There is many kinds of homelab so I need to have a global definition of what it
is. A homelab as it names imply is a laboratory at home. It is not only focused
on IT. Chemist, amateur radio operator, IT security and hosting can all be
considered as homelabing in the large description of the word. Homelabing is
mainly understood to be focused on IT as environment for testing, hosting your
own ISOs server or getting your own datacenter. There is two frightening
...
I strongly recommend to go take a look.

As a DevOps/DevSecOps/Platform engineer or wathever you call it nowadays,
I want to experiment on new technology and host my own contents like my own ISOs
server, apps like Google apps (to ungoogle and/or unicloud), manage my
...
The network planification is a fundational part of a working infrastructure.
...
- guest: Network for guests. Give access to Internet and some choosen services.

### Hardware

To have the proper material is a key to a longterm solution. As I am not part of
the 1%, I will go with what I can but I will add a little bit of challenges. I
would like my stack to take less energy and make less sound that is possible. To
be cost effective over that, will require to think with a large and far view of
the project.

This is what I have for now:

- The modem is provided by my Internet Service Provider (ISP) so not really a
lot I can do here unless getting an hardware that can replace it that is more
proficient.
- The router is a fanless Topton 12th gen with six 2.5G ethernet ports. It runs
...
server and works as a personal letsencrypt with my own selfsigned certificate.
...
surpport to no professional hardware. It helps to fix an issue without moving my
...
- The Access Point (AP or wifi antena) is also a Unifi Access Point U6 plus. Can
cover probably any needs in a modest apartment.
...
1. [Setup the router](#step-one-setup-the-router)
2. [Start the switch](#step-two-start-the-switch)
3. [The CA server](#step-three-the-ca-server)

#### Step one: Setup the router

The first step would be to setup the router. I made the choice of
[Opnsense](https://opnsense.org/) for many good reasons:

- Known to be pretty stable
- Firewall focused
- With a good enough UI
- Accessible to people less confortable with network stuff
...
Not much to do here for now as we need a Unifi controller to manage the switch.
...
Ok now its finally starting to get interesting. I went with the following
tutorial on this one
[Build a Tiny Certificate Authority For Your Homelab](https://smallstep.com/blog/build-a-tiny-ca-with-raspberry-pi-yubikey/)
and did all the optional parts because its cool. I choose to go with dietpi as
the OS.
...
##### CA server the Ansible way: Setup the SD card
...
##### CA server the Ansible way: Setup and configure our CA server

All the boring stuff is done, now it is the good and fun part. We will setup a
...
about how to setup the configuration. Run [setup.yaml](./ansible/setup.yaml).
...
The server is up and running and we just need to play with it to ensure it works
as expected. We can first get the root CA via the url by doing a `curl
--insecure https://YOUR_CA_SERVER_HOSTNAME/roots.pem`. We can also install
cerbot locally and try to obtain a certificate. We can setup the Opnsense router
to ask for a
[certificate](https://homenetworkguy.com/how-to/replace-opnsense-web-ui-self-signed-certificate-with-lets-encrypt/).

> ![NOTE]
> We will need to setup a local Bind server to answer the step-ca call for
> internal domain.

TODO: Document on how to setup Bind with step-ca to obtain our self-signed
certificate.
